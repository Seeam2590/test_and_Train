#Set working directory, load data files and the ggplot2 for visualization
...
#Then we create a model (using polynomial which gave lowest MSE) on training set and then testing it on the test
...
ggplot()+geom_point(data=train, aes(x=x,y=y))+ggtitle("Line of best fit on Training Set")+geom_line(data=testline, aes(x=x,y=y),color='red')
ggplot()+geom_point(data=test, aes(x=x,y=y))+ggtitle("Line of best fit on Test Set")+geom_line(data=testline, aes(x=x,y=y),color='red')
...
data$group = 1:1000
means$group = 1:1000
groups = rbind(data, means)   #This makes new data frame with 'data' piled over 'means'
